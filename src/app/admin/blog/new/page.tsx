...
import { Clock, Eye, Search, FileText, Settings2 } from "lucide-react";
...
function computeSeoScore(data: {
  ...
  if (!data.metaTitle) { issues.push("Missing meta title"); score -= 15; }
  else if (data.metaTitle.length > 60) { issues.push("Meta title too long (>60 chars)"); score -= 5; }

  if (!data.metaDescription) { issues.push("Missing meta description"); score -= 15; }
  else if (data.metaDescription.length > 160) { issues.push("Meta description too long (>160 chars)"); score -= 5; }
  else if (data.metaDescription.length < 50) { issues.push("Meta description too short (<50 chars)"); score -= 5; }

  if (!data.primaryKeyword) { issues.push("No primary keyword assigned"); score -= 10; }
  else if (data.content && !data.content.toLowerCase().includes(data.primaryKeyword.toLowerCase())) {
    issues.push("Primary keyword not found in content"); score -= 10;
  }

  if (!data.featuredImageAlt) { issues.push("Missing featured image alt text"); score -= 10; }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center gap-3">
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" checked={formData.noindex} onChange={(e) => setFormData({ ...formData, noindex: e.target.checked })} className="sr-only peer" />
              ...
              <div className="flex items-center gap-3">
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" checked={formData.nofollow} onChange={(e) => setFormData({ ...formData, nofollow: e.target.checked })} className="sr-only peer" />
                  <div className="w-11 h-6 bg-surface rounded-full peer peer-checked:bg-orange-500 transition-colors after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full" />
                </label>
          ...
          <div className="space-y-4">
            ...
            <FormField label="Tags">
              ...
              <FormInput
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") { e.preventDefault(); addTag(tagInput, "tags"); setTagInput(""); }
                }}
