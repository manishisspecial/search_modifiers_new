...
export default function EditBlogPostPage() {
  ...
  const handleRevert = async (versionId: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/blog/${params.id}/versions`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
            ...
            <FormField label="Meta Tags / Keywords">
              <div className="flex flex-wrap gap-2 mb-2">
                {formData.metaTags.map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-surface text-sm text-foreground"
                  >
                    {tag}
                    <button
                      type="button"
                      onClick={() => removeTag(tag, "metaTags")}
            ...
            <div className="glass rounded-xl p-4 border border-border">
              ...
              {headings.length > 0 ? (
                ...
                <p className="text-sm text-muted">
                  No headings found. Use ## and ### in your content.
                </p>
              )}
            </div>
