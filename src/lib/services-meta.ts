...
export type ServiceKpi = {
  ...
  value: string;
  delta?: string;
  icon?: "search" | "target" | "trending-up" | "users" | "star" | "zap" | "eye" | "share" | "chart" | "shield";
};

export type ServiceChannel = {
  label: string;
  pct: number;
  /** Tailwind gradient (from + to) class fragment, e.g. "from-orange-400 to-rose-500" */
  tint: string;
};
...
export type ServiceMeta = {
  /** Compact pill copy shown above the hero badge */
  pill: string;
  /** Visual kind chosen for the hero 3D mockup. */
  visual: ServiceVisualKind;
  /** Title shown in the 3D dashboard top-bar */
  dashTitle: string;
  /** Three small KPI tiles inside the dashboard */
  kpis: [ServiceKpi, ServiceKpi, ServiceKpi];
  /** Headline number + delta below KPIs */
  headlineLabel: string;
  headlineValue: string;
  headlineDelta: string;
  /** Three channel/list rows beneath the trend chart */
  channels: [ServiceChannel, ServiceChannel, ServiceChannel];
  /** Two floating callout chips around the dashboard */
  callouts: [string, string];
  /** Stat ribbon shown right below the hero */
  proof: [ServiceProof, ServiceProof, ServiceProof, ServiceProof];
  /** Slugs to surface in the related services rail (3-4 items). */
  related: ServiceSlug[];
};
...
const FALLBACK: ServiceMeta = servicesMeta["digital-marketing"];

export function getServiceMeta(slug: string): ServiceMeta {
  return (servicesMeta as Record<string, ServiceMeta>)[slug] ?? FALLBACK;
}
