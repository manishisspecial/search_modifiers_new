      ...
      <Container className="relative pt-20 pb-12 sm:pt-24 sm:pb-16">
        ...
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-6">
          <div className="lg:col-span-2">
            <h3 className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Get in touch</h3>
            <ul className="mt-5 space-y-4 text-sm text-slate-400">
              ...
              <li className="flex items-center gap-2.5">
                ...
                <a href={`tel:${site.phoneTel}`} className="hover-underline transition hover:text-white">{site.phone}</a>
              </li>
              <li className="flex items-center gap-2.5">
                <Mail className="h-4 w-4 shrink-0 text-cyan-400/70" />
                <a href={`mailto:${site.email}`} className="hover-underline transition hover:text-white">{site.email}</a>
              </li>
            </ul>
            <div className="mt-8 flex flex-wrap gap-2.5">
              {[
                ...
                <a
                  ...
                  className="rounded-full border border-white/[0.08] bg-white/[0.03] px-4 py-2 text-xs font-medium text-slate-400 transition-all duration-300 hover:border-cyan-500/20 hover:bg-white/[0.06] hover:text-white"
                >
                  {s.label}
                </a>
              ))}
          ...
          {footerColumns.slice(0, 4).map((col) => (
            <div key={col.title}>
              <h3 className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{col.title}</h3>
              <ul className="mt-5 space-y-2.5">
                {col.links.map((l) => (
                  <li key={l.href}>
                    <Link
                      href={l.href}
                      className="group inline-flex items-center gap-1 text-sm text-slate-400 transition-colors duration-300 hover:text-cyan-300"
                    >
